...
# NPC Generator (FoundryVTT v13.351 + dnd5e 5.3.0)
...
- **NPC 頭像/Token 圖片**：請使用 `modules/npc-generator/assets/portraits/<Race>/<Type>/<gender>.webp`，並在 `data/config/config.json` 設定 `races[*].raceTypeImages.<Type>.male|female|other`（例如 `modules/npc-generator/assets/portraits/Human/Guard/male.webp`）。性別鍵名需固定為 `male`、`female`、`other`。  
